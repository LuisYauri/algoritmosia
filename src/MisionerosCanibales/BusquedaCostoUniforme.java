...
public class BusquedaCostoUniforme
{
     ...
     */
    public static void buscar(int[] ubicacion, boolean d)
    {
        NodoDeBusqueda raiz = new NodoDeBusqueda(new EstadoMisioneroCanibal(ubicacion));
            ...
            if (!nodoTemp.getEstadoActual().esMeta()) // si nodoTemp no es una meta
            {
                ...
                for (int i = 0; i < sucesoresTemp.size(); i++)
                {
                    ...
                    NodoDeBusqueda nuevoNodo = new NodoDeBusqueda(nodoTemp,
                        sucesoresTemp.get(i), 
                        nodoTemp.getCosto()+
                           ((EstadoMisioneroCanibal)nodoTemp.getEstadoActual()).
                                   costoCambioEstado((EstadoMisioneroCanibal)sucesoresTemp.get(i)),
                        ((EstadoMisioneroCanibal)sucesoresTemp.get(i)).getHeuristica());
